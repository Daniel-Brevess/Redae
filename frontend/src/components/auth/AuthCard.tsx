...
export function AuthCard({ type, onClose }: AuthCardProps) {
  ...
  return <div className="auth-card-wrap"><section className="auth-card" ref={cardRef} role="dialog" aria-modal="false" aria-labelledby={`${type}-title`}>
    ...
    {submitted && <FeedbackMessage tone="success">Formulário pronto para integração.</FeedbackMessage>}
    <form className="auth-form" onSubmit={(event) => { event.preventDefault(); setSubmitted(true) }}>
      ...
      <button className="auth-submit" type="submit">{isSignup ? 'Cadastrar' : 'Entrar'}</button>
    </form>
  </section></div>
}
