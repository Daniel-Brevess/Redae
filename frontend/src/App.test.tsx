...
describe('landing page authentication cards', () => {
  ...
  it('opens the login form and enters the private area after authentication', async () => {
    const response = new Response(JSON.stringify({
      data: { accessToken: 'token', expiresIn: 900, user: { id: '1', name: 'Student', email: 'student@example.com', role: 'STUDENT' } },
      meta: {}, traceId: 'test',
    }), { status: 200, headers: { 'Content-Type': 'application/json' } })
    vi.stubGlobal('fetch', vi.fn().mockImplementation((url: string) => {
      if (url.endsWith('/auth/refresh')) return Promise.resolve(new Response(null, { status: 401 }))
      if (url.endsWith('/profile')) return Promise.resolve(new Response(JSON.stringify({ data: { id: '1', name: 'Student', email: 'student@example.com', role: 'STUDENT' }, meta: {}, traceId: 'test' }), { status: 200, headers: { 'Content-Type': 'application/json' } }))
